...
public class PrbController {
    ...
    public R listData(@Parameter(description = "查询条件", required = true) @RequestBody PrbQuery prbQuery) {
        ...
        try {
            map = prbService.listData(
                    prbQuery.getEnodebName(), prbQuery.getBegin(), prbQuery.getEnd(), prbQuery.getField());
    ...
    public void downloadExcel(HttpServletResponse response) {
        ExcelServiceBuilder.build().pageNum(10000).downloadFile(response, Prb.class, prbService);
    }
    ...
    public void uploadExcel(HttpServletResponse response,
        ...
        ExcelServiceBuilder.build().uploadFile(response, file, Prb.class, prbService, prbMapper);
    }
}
